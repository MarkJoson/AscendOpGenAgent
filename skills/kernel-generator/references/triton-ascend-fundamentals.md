...
- **Ascend -inf 保护**：在 Online Softmax 中，`tl.exp` 涉及 `-inf` 时必须用 `tl.where` 强制归零，否则产生 NaN 污染累加器（详见 `triton-ascend-optimization-techniques.md` 技巧 1）
...
# Ascend 必须：exp 后对 -inf 路径归零
scores = tl.where(max_val == float("-inf"), 0.0, scores)
